...
- **Every PR uses `.github/PULL_REQUEST_TEMPLATE.md`** — GitHub prefills it, so fill every section in place rather than writing a freeform description: Type (one of new feat / refactor / fix / chore), a one-paragraph Summary, a verb-leading Changelog (no implementation detail — these bullets are what reports are built from), Migration / Env (tick what applies, document new env vars in `.env.example`), Breaking Changes / Feature Impact, the Verification checklist (`tsc`, lint, tests, prod build — tick only what you actually ran), and Follow-up. Write "None" rather than deleting a section that doesn't apply.
- **Every review uses `.github/PULL_REQUEST_REVIEW_TEMPLATE.md`** — report findings as a table and tag each one with a severity from its legend: **P0** blocker (data loss, security hole, outage, broken build — must fix before merge), **P1** high (significant incorrect behavior — fix or explicitly track as a follow-up before merge), **P2** medium (misleading UX, missing test coverage — this PR or a quick follow-up), **P3** low (dead code, redundant guards — nice-to-have). Anything P1 or worse that is left unfixed needs a linked follow-up before the PR merges.
