...
| RabbitMQ | Worker, and the api only when it publishes | The api starts and serves HTTP without it, because the connection is opened on first use rather than at boot. Publishing fails with a queue error and reconnects on the next attempt. The worker waits for the broker at startup, and exits if the connection is later lost, so the orchestrator restarts it into that same wait |
